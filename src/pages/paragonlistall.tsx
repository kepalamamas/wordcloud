...
const ParagonList: React.FunctionComponent<IParagonListProps> = (props) => {
  ...
  useEffect(() => {
    if (!isLoaded) {
      axios
        .get(`https://konseruntuk.online/data1?_sort=id&_order=desc`)
        .then((res) => {
          setData1(res.data);
        })
        .catch((err) => console.log(err));
      axios
        .get(`https://konseruntuk.online/data2?_sort=id&_order=desc`)
        .then((res) => {
          setData2(res.data);
        })
        .catch((err) => console.log(err));
      axios
        .get(`https://konseruntuk.online/data3?_sort=id&_order=desc`)
        .then((res) => {
          setData3(res.data);
        })
        .catch((err) => console.log(err));
