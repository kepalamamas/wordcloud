...
const ParagonList: React.FunctionComponent<IParagonListProps> = (props) => {
  ...
  useEffect(() => {
    if (!isLoaded) {
      axios
        .get(`https://kuksh.cloud/api/data1?_sort=id&_order=desc`)
  ...
  const handleDeleteAll = () => {
    ...
    if (deleteKataAll) {
      data1.forEach((data) => {
        axios
          .delete(`https://kuksh.cloud/api/data1/${data.id}`)
          .catch((err) => console.log(err));
      });
  ...
  const handleDelete = (id: any) => {
    ...
    if (deleteKata) {
      axios
        .delete(`https://kuksh.cloud/api/data1/${id}`)
        .catch((err) => console.log(err));
    }
  };
      ...
      <div className="container">
        ...
        <div className="row">
          <div className="list-holder">
            <h2>List 1 Kata buat 2025</h2>
            <h3>Total: {data1.length}</h3>
